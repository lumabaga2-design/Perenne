const interaccionesModulo = {
    ...
    async darLike(itemId) {
        try {
            const respuesta = await fetch('http://localhost:3000/api/interactions/like', {
                method: 'POST',
    ...
    async crearComentario(itemId, texto) {
        try {
            const respuesta = await fetch('http://localhost:3000/api/interactions/comment', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    user_id: 1,
    ...
    async compartir(itemId) {
        try {
            const respuesta = await fetch('http://localhost:3000/api/interactions/share', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    user_id: 1,
            ...
            if (respuesta.ok) {
                const url = `http://localhost:3000/shared/${datos.share_code}`;
