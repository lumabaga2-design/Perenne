const notificacionesModulo = {
    ...
    async cargarNotificaciones() {
        try {
            const respuesta = await fetch(`http://localhost:8080/api/notifications/user/${this.usuarioActualId}`);
    ...
    async marcarLeidda(notificationId) {
        try {
            const respuesta = await fetch(`http://localhost:8080/api/notifications/${notificationId}/read`, {
                method: 'PUT'
            });
